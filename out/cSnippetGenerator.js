...
exports.CSnippetGenerator = void 0;
...
class CSnippetGenerator {
    ...
    getCandidatesForStates(states) {
        ...
        const fileName = path.join(__dirname, "../src/c11-data-state-collection.txt");
        ...
        try {
            ...
            link.on("data", data => {
                ...
                candidates.forEach((item, index) => {
                    item.sortText = (index + 1).toString().padStart(3, "0"); // Ranking starts from 1
                });
    ...
    // A function that passes the value that needs to be passed to the cparser server.
    getCompletionItems() {
        if (link === null) {
            return;
        }
    ...
    /**
     * A function that takes a completionItem and returns a code snippet conforming to C syntax.
    ...
    async getInsertText(completionItem, resulted_prefix) {
        ...
        if (resulted_prefix === "codecompletion") {
            ...
            const prompt = `
                This is the incomplete C programming language code:
                ${resulted_prefix}
                '${modifiedStructCandi}'
                Complete the '${modifiedStructCandi}' part of the code in the C programming language. Just show your answer in place of '${modifiedStructCandi}'. Do not produce multiple lines. Stick your answer to the given structure. 
                `;
...
exports.CSnippetGenerator = CSnippetGenerator;
//# sourceMappingURL=cSnippetGenerator.js.map